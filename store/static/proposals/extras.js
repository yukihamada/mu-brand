 ...
 *   - 30 / 50 / 100 buttons (30 is free first time per email)
 *   - email input + balance display
...
(function () {
  ...
  // Skip the swipe variant pages — they have their own UI.
  if (/-swipe$/i.test(SLUG)) return;
  ...
  // ── Render ────────────────────────────────────────────────────────────
  var section = $$("div", { id: "mu-extras-section", style:
  ...
  section.innerHTML = ''
    ...
    + '<p style="font-size:14px;color:rgba(245,245,240,0.62);line-height:1.95;margin-bottom:18px">'
    +   '物足りなければ AI で <strong style="color:#fff">30 / 50 / 100</strong> 種類の新 SKU を自動生成できます。'
    +   '初回 30 個は <strong style="color:#7be57b">無料</strong>。 2 回目以降は <strong style="color:#fff">1 商品 = 10pt</strong> を消費 — '
    +   'ポイントは <strong style="color:#fff">MUGEN T シャツ購入で +1,000pt</strong>、 サンプル購入で <strong style="color:#fff">購入額の 10%</strong> がポイント還元されます。'
    + '</p>'
    ...
    +     '<div style="font-size:11px;letter-spacing:0.22em;text-transform:uppercase;color:#7be57b;font-weight:700;margin-bottom:8px">生成中</div>'
    ...
    +     '<div id="mu-ex-job-items" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px;margin-top:14px"></div>'
    +   '</div>'
  ...
  // Style the qty buttons consistently.
  function applyQtyButtonStyle (b, recommended) {
    b.style.cssText =
      ...
      + "border-radius:3px;cursor:pointer;line-height:1.5;text-align:center;"
      + "font-family:inherit;";
  }
  ...
  function paintQtyButtons (bal) {
    ...
    section.querySelectorAll(".mu-ex-qty").forEach(function (btn) {
      ...
      btn.innerHTML = '<div style="font-size:20px;letter-spacing:0.04em;margin-bottom:4px">+' + qty + '</div>'
        ...
        + (isFree ? '無料（初回 1 回のみ）' : cost.toLocaleString() + ' pt')
        + '</div>';
    });
  }
  ...
  function orderExtras (qty) {
    ...
    api("/api/proposal/" + SLUG + "/extras/quote", { email: em, qty: qty }).then(function (q) {
      ...
      var msg = "MU × " + SLUG.toUpperCase() + " の SKU を " + qty + " 個追加します。 (" + costTxt + ")";
      if (!q.sufficient) {
        setStatus(msg + " — ポイントが " + q.shortfall.toLocaleString()
    ...
    function tick () {
      ...
      api("/api/proposal/extras/job/" + jobId).then(function (j) {
        ...
        if (Array.isArray(j.items)) {
          var current = jobItems.children.length;
          for (var i = current; i < j.items.length; i++) {
            var it = j.items[i];
            var card = document.createElement("a");
            card.href = it.image_url || "#";
            card.target = "_blank";
            card.style.cssText = "background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.06);border-radius:3px;overflow:hidden;display:block;text-decoration:none;color:inherit";
            card.innerHTML = (it.image_url
                ? '<img src="' + it.image_url + '" alt="" style="width:100%;height:auto;display:block">'
        ...
        if (j.status === "completed" || j.status === "failed" || j.status === "partial") {
          ...
          if (j.status === "completed") {
            jobMsg.innerHTML = '<span style="color:#7be57b">全 ' + j.total + ' 商品の生成が完了しました。 ページを再読込みすると下の SKU 一覧に並びます。</span>';
          } else if (j.status === "partial") {
            jobMsg.innerHTML = '<span style="color:#e6c449">部分完了: ' + j.done + ' / ' + j.total + ' 個生成。 残り分のポイントは自動返却されました。</span>';
          } else {
            jobMsg.innerHTML = '<span style="color:#ff8a8a">失敗しました。 ポイントは全額返却されました。 ' + (j.last_error || "") + '</span>';
          }
          return;
        }
