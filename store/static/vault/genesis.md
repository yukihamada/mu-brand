...
- 公開 7 日経過、 **累計販売: 8 枚**
- うち**純外部顧客**: 1 名 (Kenny / Hawaii / 第 1 回 Founder Relay 受け取り手)
...
経過 (本記録時点 = 2026-05-18 / 公開 11 日目、`/transparency` 同時刻 snapshot):

- **累計購入: 13 件** (Stripe live `cs_live_*` のみ)
- **累計実売上: ¥89,000**
- **出荷済み枚数: 11 枚** (`products.sold` 合計)
- **累計デザイン生成: 125** (`products + you_designs` の行数)
