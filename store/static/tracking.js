/* MU tracking shim — Google Analytics 4 + Google Ads conversion.
   Tags only emit when /api/tracking/config returns non-empty IDs, so the
   file is safe to embed everywhere even before Ads is set up.
   ...
   on the server, calls are no-ops. */
(function () {
  ...
  function purchase(d) {
    if (!cfg.ga4 && !cfg.gads) { queue.push([purchase, [d]]); return; }
  ...
  function lead(d) {
    if (!cfg.ga4 && !cfg.gads) { queue.push([lead, [d]]); return; }
    const value = Number(d && d.value) || 0;
    const currency = (d && d.currency) || 'JPY';
