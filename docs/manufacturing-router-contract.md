...
| `route` | str | `fulfillment_route` 値。`*` kinds は kind 毎に `route_for_kind` で解決 |
...
## エンドポイント / ツール
- `GET /api/agent/quote?kind=&description=&qty=&region=&budget=` — 公開(auth不要)・read-only。
- MCP `mu_quote` — 同上を叩く。auto なら `mu_create_product` へ、quote なら人手 RFQ(Phase2)。
